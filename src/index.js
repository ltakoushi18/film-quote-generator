...
function generateFilmQuote(event) {
  ...
  let context =
    "You are a film expert that knows quotes from lots of films. Your mission is to generate just the well-known quote from the chosen film. Make sure to follow user instructions";
  let prompt = `User instructions: Generate a film quote from ${instructions.value}`;
  let apiUrl = `https://api.shecodes.io/ai/v1/generate?prompt=${prompt}&context=${context}&key=${apiKey}`;

  let quoteHidden = document.querySelector("#insert-quote");
